...
{
public:
    IHdlc(hdlc_handle_t handle): m_hdlc( handle ) { }
    ...
    virtual int onRxFrame( char *buf, int size ) { return 0; };

    int runTx();

    int runRx();

    int runRx(const char *buf, int size);
...
protected:
    hdlc_handle_t        m_hdlc = nullptr;

    read_block_cb_t      m_readcb = nullptr;
